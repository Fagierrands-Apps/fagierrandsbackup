...
class AllClientsListView(generics.ListAPIView):
    """
    API endpoint to list ALL clients in the system
    Returns all users with user_type='user' or 'client'
    """
    serializer_class = UserSerializer
    permission_classes = [IsHandler]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['is_verified', 'email_verified']
    search_fields = ['username', 'first_name', 'last_name', 'email', 'phone_number']
    ordering_fields = ['date_joined', 'first_name', 'last_name']
    ordering = ['-date_joined']
    pagination_class = None  # No pagination - return all clients
...
@api_view(['POST'])
@permission_classes([permissions.IsAuthenticated])
def handler_create_user(request):
    """
    Allow handlers to create users (clients, assistants, or other handlers).
    Only authenticated handlers can access this endpoint.
    """
    # Check if the requesting user is a handler
    if request.user.user_type != 'handler':
        return Response({
            'error': 'Permission denied. Only handlers can create users.'
        }, status=status.HTTP_403_FORBIDDEN)
    ...
    try:
        ...
        if not all([phone_number, first_name, last_name, password]):
            return Response({
                'error': 'Missing required fields: phone_number, first_name, last_name, password'
            }, status=status.HTTP_400_BAD_REQUEST)
        
        # Validate user_type
        valid_user_types = ['client', 'assistant', 'handler']
        ...
        user = User.objects.create_user(
            ...
            user_type=user_type,
            password=password,
            is_verified=True,  # Handler-created users are pre-verified
            email_verified=True if email else False,
            account_manager=request.user  # Assign the handler as account manager
        )
        
        logger.info(f"Handler {request.user.id} created user {user.id} ({user_type})")
