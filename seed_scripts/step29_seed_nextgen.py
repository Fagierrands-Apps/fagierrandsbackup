...
# Seed nextgen if not exists
print(">>> Checking/seeding nextgen...")
psql(f"""
    ...
    VALUES ('!disabled', NULL, false, 'nextgenmall', 'Nextgen', 'Mall',
         'nextgenmall@fagierrands.com', false, true, '{now}', 'client', '',
         true, true, '{now}', '{now}', false)
    ON CONFLICT (username) DO NOTHING
    RETURNING id, username;
""")
psql("INSERT INTO accounts_profile (bio, address, user_id, wallet_balance, wallet_points) SELECT '', '', id, 0, 0 FROM accounts_user WHERE username='nextgenmall';")
psql("SELECT id FROM accounts_user WHERE username='nextgenmall';")
