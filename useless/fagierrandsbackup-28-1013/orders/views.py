...
def get_client_user(request):
    ...
    if client_id and requester.user_type in ['handler', 'admin']:
        try:
            ...
            # Handlers and admins can place orders for any client
            return client
...
class PickupDeliveryOrderCreateView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = PickupDeliveryOrderSerializer(data=data)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                image_count = 0
                if request.FILES:
                    from accounts.cloudinary_service import CloudinaryService
                    from orders.models import OrderImage
                    cloudinary_service = CloudinaryService()
                    
                    images = request.FILES.getlist('images')
                    for idx, image_file in enumerate(images):
                        try:
                            # Upload to Cloudinary with order ID in folder
                            result = cloudinary_service.upload_file(
                                image_file,
                                folder=f'orders/order_{order.id}',
                                public_id=f'image_{idx+1}'
                            )
                            if result[0]:  # Success
                                # Create OrderImage record
                                OrderImage.objects.create(
                                    order=order,
                                    image=result[1]  # Cloudinary URL
                                )
                                image_count += 1
                                logger.info(f"Uploaded image {idx+1} for order {order.id}: {result[1]}")
                            else:
                                logger.error(f"Failed to upload image {idx+1} for order {order.id}: {result[1]}")
                        except Exception as e:
                            logger.error(f"Error uploading image {idx+1} for order {order.id}: {str(e)}")
